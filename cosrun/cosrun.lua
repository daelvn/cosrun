...
elseif "run" == _exp_0 then
  ...
  if config.wsl.use then
    ...
    util.arrow("Running...")
    local command = "\"" .. tostring(config.executable) .. "\" --directory '" .. tostring(util.toWSLPath((util.absolutePath(at)), config.wsl.prefix)) .. "' --script .cosrun/" .. tostring(config.env) .. "/mount.lua " .. tostring(config.flags)
    print("   " .. command)
    os.execute(command)
    ...
    util.safeCopy(root, tostring(at) .. "computer/" .. tostring(self.id))
    util.arrow("Running...")
    local command = "\"" .. tostring(config.executable) .. "\" --directory '" .. tostring(at) .. "' --script .cosrun/" .. tostring(config.env) .. "/mount.lua " .. tostring(config.flags)
    print("   " .. command)
    os.execute(command)
    util.arrow("Copying ID " .. tostring(self.id) .. " to root")
    util.safeCopy(tostring(at) .. "/computer/" .. tostring(self.id), root)
